...
public class TestRobot {
    ...
    public void testMoveWhenMultipleMoves() {
        Planet mars = new Planet(10,5);
        Robot rory = new Robot(mars, 5,0,'E');
        String position = rory.move("FFFLFFFFFFF");
        assertEquals("Current position unexpected", "8 4 N LOST", position);
        assertEquals("Number of scents unexpected", 1, mars.totalScents());
        assertTrue("Expeected position to be scented", mars.isScented(8,4));
    }
    ...
    public void testMoveWhenScented() {
        ...
        String position = rory.move("FFFFFRFFFFFFFFFF");
        assertEquals("Current position unexpected", "9 5 E LOST", position);
        assertEquals("Number of scents unexpected", 1, mars.totalScents());
        assertTrue("Expeected position to be scented", mars.isScented(9,5));
        Robot walle = new Robot(mars, 5,5,'E');
        position = walle.move("FFFFFF");
        assertEquals("Current position unexpected", "9 5 E", position);
        assertEquals("Number of scents unexpected", 1, mars.totalScents());
        assertTrue("Expeected position to be scented", mars.isScented(9,5));
    }
    ...
    public void testMoveWhenScentedAndMoveAfterHittingScent() {
        ...
        assertEquals("Current position unexpected", "4 9 W", position);
        assertEquals("Number of scents unexpected", 1, mars.totalScents());
        assertTrue("Expeected position to be scented", mars.isScented(9,5));
    }
    ...
    public void testMoveWhenScentedAndMoveAfterHittingScentAndLost() {
        ...
        assertTrue("Expeected position to be scented", mars.isScented(9,5));
        Robot walle = new Robot(mars, 5,5,'E');
        position = walle.move("FFFFFFLFFFFLFFFFFFFFFFFF");
        assertEquals("Current position unexpected", "0 9 W LOST", position);
        assertEquals("Number of scents unexpected", 2, mars.totalScents());
        assertTrue("Expeected position to be scented", mars.isScented(9,5));
        assertTrue("Expeected position to be scented", mars.isScented(0,9));
    }
}
